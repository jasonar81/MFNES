...
public class PPU implements Serializable  {
	...
	public void setPpuAddr(int val)
	{
		...
		if (ppuAddr >= 0x1000 && ppuAddr < 0x2000)
		{
			...
			{
				if (((Mmc3Port)cpu.getMem().getLayout()[0x8000]).getMmc3Data().counter == 0)
				{
					if (((Mmc3Port)cpu.getMem().getLayout()[0x8000]).getMmc3Data().irqEnabled)
					{
						cpu.setIrq();
					}
					
					((Mmc3Port)cpu.getMem().getLayout()[0x8000]).getMmc3Data().counter = ((Mmc3Port)cpu.getMem().getLayout()[0x8000]).getMmc3Data().irqReloadValue;
				}
				else if (((Mmc3Port)cpu.getMem().getLayout()[0x8000]).getMmc3Data().needsReload)
				{
					((Mmc3Port)cpu.getMem().getLayout()[0x8000]).getMmc3Data().counter = ((Mmc3Port)cpu.getMem().getLayout()[0x8000]).getMmc3Data().irqReloadValue;
				} else
				{
					--((Mmc3Port)cpu.getMem().getLayout()[0x8000]).getMmc3Data().counter;
				}
				
				((Mmc3Port)cpu.getMem().getLayout()[0x8000]).getMmc3Data().needsReload = false;
			}
		}
	}
