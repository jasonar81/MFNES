...
public class GenPunchOut3Video implements AiAgent {
	private Clock clock;
	private CPU cpu;
	private PPU ppu;
	private APU apu;
	...
	private static GenPunchOut3Video instance;
	...
	{
		instance = new GenPunchOut3Video();
		String dir = args[0];
		String ts = args[1];
	...
	{	
		...
		//Run
		loadNet(dir + "punch_out_pixels" + ts + ".net");
		setup();
	...
	private boolean loadNet(String filename)
	{
		try
		{
			...
			net = new ControllerNeuralNet(true, layerSize, numLayers, false, true);
	...
	private void setup()
	{
		...
		clock = new Clock();
		gui = new RecordingNetGui(true, numControllerRequests, firstUsableCycle, net, startOnOffTimes, clock, dir + "punch_out_pixels" + ts + ".mp4");
