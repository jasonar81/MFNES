...
public class BubbleBobble2PMultiTreeScenes implements AiAgent {
	private Clock clock;
	private CPU cpu;
	private PPU ppu;
	private APU apu;
	...
	private static BubbleBobble2PMultiTreeScenes instance;
	...
	private long numControllerRequests = 160000;
	...
	{
		instance = new BubbleBobble2PMultiTreeScenes();
		instance.main();
	}
	...
	private void main()
	{
		...
		if (!loadTree())
		{
			...
			tree = new TwoPlayerMultiDecisionTreeWithScenes(validStates, addresses, root, disallow);
		}
		...
		while (true)
		{
			numControllerRequests = usedControllerRequests + 160000;
			setup();
			load("bubble_bobble.nes", "sav");
			makeModifications();
			controller.reset();
	...
	private void setup(Snapshot snap)
	{
		...
		((NotifyChangesPort)cpu.getMem().getLayout()[0x2e]).setAgent(this);
		((NotifyChangesPort)cpu.getMem().getLayout()[0x42]).setAgent(this);
		((SaveAndUpdateMaxValuePort)cpu.getMem().getLayout()[0x401]).setAgent(this); 
	...
	public synchronized void progress(long cycle)
	{
		...
		{
			if (cpu.getMem().getLayout()[0x2e].read() == 2)
			{
				setDone(cycle);
				return;
			}
			
			if (cpu.getMem().getLayout()[0x42].read() == 2)
			{
				setDone(cycle);
				return;
			}
