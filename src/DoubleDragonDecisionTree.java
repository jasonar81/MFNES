...
public class DoubleDragonDecisionTree implements AiAgent {
	private Clock clock;
	private CPU cpu;
	private PPU ppu;
	private APU apu;
	...
	private static DoubleDragonDecisionTree instance;
	...
	{
		instance = new DoubleDragonDecisionTree();
		instance.main();
	}
	...
	private boolean loadTree()
	{
		try
		{
			File file = new File("double_dragon.tree");
			if (!file.exists())
			{
				return false;
			}
	...
	private boolean loadTree2()
	{
		try
		{
			File file = new File("double_dragon.tree2");
			if (!file.exists())
			{
				return false;
			}
	...
	public synchronized void progress(long cycle)
	{
		...
		{
			if (cpu.getMem().getLayout()[0x43].read() == (byte)0xff)
			{
				setDone(cycle);
				return;
			}
