...
public class AdventureIsland3DecisionTree implements AiAgent {
	private Clock clock;
	private CPU cpu;
	private PPU ppu;
	private APU apu;
	...
	{
		instance = new AdventureIsland3DecisionTree();
		instance.main();
	}
	...
	private boolean loadTree()
	{
		try
		{
			File file = new File("adventure_island3.tree");
			if (!file.exists())
			{
				return false;
			}
	...
	private boolean loadTree2()
	{
		try
		{
			File file = new File("adventure_island3.tree2");
			if (!file.exists())
			{
				return false;
			}
