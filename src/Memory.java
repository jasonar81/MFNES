...
private static final long serialVersionUID = -6732487624928621347L;
	...
	private void configureCpuMem()
	{
		...
		layout[0x4014] = new Register4014(cpu);
	...
	private void mapper1()
	{
		...
		try
		{
			...
			Mmc1Data mmc1Data = new Mmc1Data();
			for (int i = 0x8000; i < MEMSIZE; ++i)
			{
				layout[i] = new Mmc1Port(this, i, layout[i], mmc1Data);
			}
		}
		catch(Exception e)
		{
	...
	private void mapper4()
	{
		...
		try
		{
			...
			Mmc3Data mmc3Data = new Mmc3Data();
			for (int i = 0x8000; i < MEMSIZE; ++i)
			{
				layout[i] = new Mmc3Port(this, i, layout[i], cart, mmc3Data);
			}
		}
		catch(Exception e)
		{
	...
	private void mapper1Ppu()
	{
		...
		{
			layout[i] = new Mmc1NametablePort((i - 0x2000) & 0x7ff, ((Mmc1Port)cpu.getMem().getLayout()[0x8000]).getMmc1Data());
		}
	}
	...
	private void mapper4Ppu()
	{
		...
		for (int i = 0x2000; i < 0x3000; ++i)
		{
			layout[i] = new Mmc3NametablePort((i - 0x2000) & 0x7ff, ((Mmc3Port)cpu.getMem().getLayout()[0x8000]).getMmc3Data());
		}
	}
