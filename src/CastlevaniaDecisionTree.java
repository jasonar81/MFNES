...
public class CastlevaniaDecisionTree implements AiAgent {
	private Clock clock;
	private CPU cpu;
	private PPU ppu;
	private APU apu;
	...
	private static CastlevaniaDecisionTree instance;
	...
	{
		instance = new CastlevaniaDecisionTree();
		instance.main();
	}
	...
	private boolean loadTree()
	{
		try
		{
			File file = new File("castlevania.tree");
			if (!file.exists())
			{
				return false;
			}
	...
	private boolean loadTree2()
	{
		try
		{
			File file = new File("castlevania.tree2");
			if (!file.exists())
			{
				return false;
			}
