...
public class BubbleBobble2PMultiTree implements AiAgent {
	private Clock clock;
	private CPU cpu;
	private PPU ppu;
	private APU apu;
	...
	private static BubbleBobble2PMultiTree instance;
	...
	private TwoPlayerMultiDecisionTree tree;
	private TwoPlayerMultiTreeController controller;
	...
	{
		instance = new BubbleBobble2PMultiTree();
		instance.main();
	}
	...
	private void main()
	{
		...
		if (!loadTree())
		{
			...
			tree = new TwoPlayerMultiDecisionTree(validStates, addresses, root, disallow);
		}
		
		controller = new TwoPlayerMultiTreeController(tree);
	...
	private boolean loadTree()
	{
		try
		{
			File file = new File("bubble_bobble2p_scenes.tree");
			if (!file.exists())
			{
				return false;
			}
			...
			tree = (TwoPlayerMultiDecisionTree)i.readObject();
			tree.makeWhole();
	...
	private void setup()
	{
		...
		clock = new Clock();
		gui = new TwoPlayerMultiTreeGui(numControllerRequests, firstUsableCycle, controller, startOnOffTimes, clock);
		long[] selectTimes = new long[] {28701596, 29323805};
		((TwoPlayerMultiTreeGui)gui).setSelectTimes(selectTimes);
	...
	private void makeModifications()
	{
		...
		cpu.getMem().getLayout()[0x42] = new NotifyChangesPort(this, clock); //Lives remaining
	...
	public void setDone(long totalTime)
	{
		if (!startedDone && !done)
		{
			...
			usedControllerRequests = ((TwoPlayerMultiTreeGui)gui).getRequests();
		}
	}
	...
	public synchronized void progress(long cycle)
	{
		...
		if (cycle >= firstUsableCycle)
		{
			...
			else if (cpu.getMem().getLayout()[0x42].read() == 2)
			{
				setDone(cycle);
				return;
			}
