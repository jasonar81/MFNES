...
private static final long serialVersionUID = -6732487624928621347L;
	...
	private transient ArrayList<Snapshot> snapshots = new ArrayList<Snapshot>();
	private AtomicBoolean snapshotRequested = new AtomicBoolean(false);
	private transient MultiTreeControllerWithScenes controller;
	private transient TwoPlayerMultiTreeControllerWithScenes twoPlayerController;
	...
	public void setController(MultiTreeControllerWithScenes controller)
	{
		this.controller = controller;
	}
	
	public void set2PController(TwoPlayerMultiTreeControllerWithScenes controller)
	{
		this.twoPlayerController = controller;
	}
	...
	{
		Snapshot src = new Snapshot();
		src.apu = apu;
		src.clock = clock;
		
		if (controller != null)
		{
			src.count = controller.getCount();
			src.state = controller.getState();
		}
		else
		{
			src.count = twoPlayerController.getCount();
			src.state = twoPlayerController.getState();
		}
		
		src.cpu = this;
		src.cpuMem = mem;
		src.ppu = ppu;
		src.ppuMem = ppu.getMem();
