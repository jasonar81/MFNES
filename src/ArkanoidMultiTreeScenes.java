...
public class ArkanoidMultiTreeScenes implements AiAgent {
	private Clock clock;
	private CPU cpu;
	private PPU ppu;
	private APU apu;
	...
	private static ArkanoidMultiTreeScenes instance;
	...
	private long numControllerRequests = 70000;
	...
	{
		instance = new ArkanoidMultiTreeScenes();
		instance.main();
	}
	...
	private void main()
	{
		...
		while (true)
		{
			numControllerRequests = usedControllerRequests + 70000;
			setup();
			load("arkanoid.nes");
			makeModifications();
			controller.reset();
	...
	private void setup(Snapshot snap)
	{
		...
		gui = new MultiTreeGuiWithScenes(70000, 0, controller, startOnOffTimes, clock);
		guiThread = new Thread(gui);
		guiThread.setPriority(10);
		guiThread.start();
		...
		((NotifyChangesPort)cpu.getMem().getLayout()[0x0d]).setAgent(this); 
